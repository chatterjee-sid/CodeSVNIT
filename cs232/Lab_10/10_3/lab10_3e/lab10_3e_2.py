...
def calculate_p_ha():
    ...
    p_hw_on_pa_sm = get_prob(cpt_hw, {'Party': 'T', 'Smart': 'T' })
    for hw in [True, False]:
        p_hw = p_hw_on_pa_sm if hw else 1 - p_hw_on_pa_sm
        p_pr_on_sm_cr = get_prob(cpt_pr, {'Smart': 'T', 'Creative': 'F'})
        for pr in [True, False]:
            ...
            for su in [True, False]:
                ...
                p_mu_on_sm_cr = get_prob(cpt_mu, {'Smart': 'T', 'Creative': 'F'})
                for mu in [True, False]:
                    p_mu = p_mu_on_sm_cr if mu else 1 - p_mu_on_sm_cr
                    p_ha_on_pa_mu_su = get_prob(cpt_ha, {'Party': 'T',
                                                         'Music': 'T' if mu else 'F',
                                                         'Success': 'T' if su else 'F'})
                    joint_prob = p_hw * p_pr * p_su * p_mu * p_ha_on_pa_mu_su
                    p_ha += joint_prob
    
    return p_ha

if __name__ == "__main__":
    print(f"P(Happy = 1 | Party = 1, Smart = 1, Creative = 0) = {calculate_p_ha():.3f}")