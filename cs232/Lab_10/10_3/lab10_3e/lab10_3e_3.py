...
def calculate_p_ha():
    ...
    for pa in [True, False]:
        ...
        p_hw_on_pa_sm = get_prob(cpt_hw, {'Party': 'T' if pa else 'F', 'Smart': 'T'})
        for hw in [True, False]:
            p_hw = p_hw_on_pa_sm if hw else 1 - p_hw_on_pa_sm
            p_pr_on_sm_cr = get_prob(cpt_pr, {'Smart': 'T', 'Creative': 'T'})
            for pr in [True, False]:
                ...
                for su in [True, False]:
                    ...
                    p_mu_on_sm_cr = get_prob(cpt_mu, {'Smart': 'T', 'Creative': 'T'})
                    for mu in [True, False]:
                        ...
                        joint_prob = p_pa * p_hw * p_pr * p_su * p_mu * p_ha_on_pa_mu_su
                        p_ha += joint_prob
    
    return p_ha

if __name__ == "__main__":
    print(f"P(Happy = 1 | Smart = 1, Creative = 1) = {calculate_p_ha():.3f}")