...
def calculate_p_ha():
    ...
    for sm in [True, False]:
        ...
        for cr in [True, False]:
            ...
            p_su_on_hw_pr = get_prob(cpt_su, {'HW': 'T', 'Project': 'T'})
            for su in [True, False]:
                ...
                for mu in [True, False]:
                    ...
                    p_ha_on_pa_mu_su = get_prob(cpt_ha, {'Party': 'F',
                                                         'Music': 'T' if mu else 'F',
                                                         'Success': 'T' if su else 'F'})
                    joint_prob = p_sm * p_cr * p_su * p_mu * p_ha_on_pa_mu_su
                    p_ha += joint_prob
    
    return p_ha

if __name__ == "__main__":
    print(f"P(Happy = 1 | Party = 0, HW = 1, Project = 1) = {calculate_p_ha():.3f}")